...
begin
	area = (
		minlat = 52.0942, minlon = 8.1711, # bottom left corner
		maxlat = 52.2258, maxlon = 8.3771 # top right corner
	)
...
function query_overpass_api(query::String)
	sleep(1.05)
	return @chain "https://overpass-api.de/api/interpreter" begin
		HTTP.get(query=Dict("data" => query))
...
# ╔═╡ 5eb5fa1b-3e15-4135-88cb-985c9942100e
bounding_box_comma_sep = "51.983,7.947,52.321,8.605"
...
function query_overpass_cities_towns_villages()
	...
	“place=* AND type:node AND (place=town OR place=village OR place=city)”
	*/
	[out:json][timeout:25];
	// gather results
	(
	  // query part for: “place=* and place=town”
	  node["place"]["place"="town"]($bounding_box_comma_sep);
	  // query part for: “place=* and place=village”
	  node["place"]["place"="village"]($bounding_box_comma_sep);
	  // query part for: “place=* and place=city”
	  node["place"]["place"="city"]($bounding_box_comma_sep);
	);
	// print results
	out body;
	>;out skel qt;
...
function query_overpass_turbo_train_stations()
	...
	This has been generated by the overpass-turbo wizard.
	The original search was:
	“(railway=station OR railway=halt) AND public_transport=station AND usage!=tourism”
	*/
	[out:json][timeout:25];
	// gather results
	(
	  // query part for: “railway=station and public_transport=station and usage!=tourism”
	  node["railway"="station"]["public_transport"="station"]["usage"!="tourism"]($bounding_box_comma_sep);
	  way["railway"="station"]["public_transport"="station"]["usage"!="tourism"]($bounding_box_comma_sep);
	  relation["railway"="station"]["public_transport"="station"]["usage"!="tourism"]($bounding_box_comma_sep);
	  // query part for: “railway=halt and public_transport=station and usage!=tourism”
	  node["railway"="halt"]["public_transport"="station"]["usage"!="tourism"]($bounding_box_comma_sep);
	  way["railway"="halt"]["public_transport"="station"]["usage"!="tourism"]($bounding_box_comma_sep);
	  relation["railway"="halt"]["public_transport"="station"]["usage"!="tourism"]($bounding_box_comma_sep);
	);
	// print results
	out body;
	>;
...
function query_overpass_turbo_l3_hospitals()
	...
	*/
	...
	// gather results
	(
	  // query part for: “"hospital:level:DE"=3”
	  node["hospital:level:DE"="3"]($bounding_box_comma_sep);
	  way["hospital:level:DE"="3"]($bounding_box_comma_sep);
	  relation["hospital:level:DE"="3"]($bounding_box_comma_sep);
	);
	// print results
	out center body;
	"""
...
function query_overpass_supermarket()
	...
	*/
	...
	(
	  ...
	  node["shop"="supermarket"]($bounding_box_comma_sep);
	  way["shop"="supermarket"]($bounding_box_comma_sep);
	  relation["shop"="supermarket"]($bounding_box_comma_sep);
	);
	// print results
	out center body;
	"""
...
d_train = query_overpass_turbo_train_stations()
...
train_geo = GeoJSON.read(union_geojson(d_train_vh))

# ╔═╡ 99d3fe4e-4799-4ed0-9557-f0ea3158c7b7
supermarket_geo = GeoJSON.read(union_geojson(d_supermarket_vh))

# ╔═╡ 0145b1df-c868-4645-90ea-a8ebd1964f0a
infra_centers_geo = GeoJSON.read(intersect_geojson(GeoJSON.write.([supermarket_geo, hospital_geo, train_geo])))
...
function plot_supermarkets()
	fig, ax, plot = build_teuto_basemap()

	poly!(
		ax,
		GeoMakie.geo2basic(supermarket_geo);
		color=("#3498eb", 0.8),
	)
	...
	ax.title = "Infrastructure Access: 15 min bike to supermarket"
	return fig, ax, plot
end

# ╔═╡ 76921d96-a525-4f9e-aeb7-9de3547d853f
# ╠═╡ show_logs = false
plot_supermarkets()[1]

# ╔═╡ 9a27843a-187f-424c-a603-be52c7ec3f4d
function plot_infra_centers()
	fig, ax, plot = build_teuto_basemap()

	poly!(
		ax,
		GeoMakie.geo2basic(infra_centers_geo);
		color=("#d69e02", 0.8),
	)
	...
	ax.title = "Infrastructure Access: Supermarket, Train & Hospital"
	return fig, ax, plot
end

# ╔═╡ c7a712fe-2fc9-4e41-ba7a-ff0483f95a24
# ╠═╡ show_logs = false
plot_infra_centers()[1]
