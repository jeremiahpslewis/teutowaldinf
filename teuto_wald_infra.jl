...
function query_overpass_api(url::String)
	sleep(1.05)
...
function query_overpass_cities_towns_villages()
	url = "https://overpass-api.de/api/interpreter?data=%2F*%0AThis%20has%20been%20generated%20by%20the%20overpass-turbo%20wizard.%0AThe%20original%20search%20was%3A%0A“place%3D*%20AND%20type%3Anode%20AND%20%28place%3Dtown%20OR%20place%3Dvillage%20OR%20place%3Dcity%29”%0A*%2F%0A%5Bout%3Ajson%5D%5Btimeout%3A25%5D%3B%0A%2F%2F%20gather%20results%0A%28%0A%20%20%2F%2F%20query%20part%20for%3A%20“place%3D*%20and%20place%3Dtown”%0A%20%20node%5B%22place%22%5D%5B%22place%22%3D%22town%22%5D%2851.97472977494965%2C7.9163360595703125%2C52.32946474208912%2C8.68743896484375%29%3B%0A%20%20%2F%2F%20query%20part%20for%3A%20“place%3D*%20and%20place%3Dvillage”%0A%20%20node%5B%22place%22%5D%5B%22place%22%3D%22village%22%5D%2851.97472977494965%2C7.9163360595703125%2C52.32946474208912%2C8.68743896484375%29%3B%0A%20%20%2F%2F%20query%20part%20for%3A%20“place%3D*%20and%20place%3Dcity”%0A%20%20node%5B%22place%22%5D%5B%22place%22%3D%22city%22%5D%2851.97472977494965%2C7.9163360595703125%2C52.32946474208912%2C8.68743896484375%29%3B%0A%29%3B%0A%2F%2F%20print%20results%0Aout%20body%3B%0A%3E%3B%0Aout%20skel%20qt%3B"
...
cities_towns = query_overpass_cities_towns_villages()
...
function query_valhalla(latitude::Float64, longitude::Float64;
	costing="bicycle", time=15.0)
	sleep(1.05)
	...
	return @chain "https://valhalla1.openstreetmap.de/isochrone" begin 
		HTTP.get(query=Dict("json" => JSON.json(json_payload)
			))
		@aside @assert(_.status == 200)
		_.body
		String
...
function intersect_geojson(geo_json_1, geo_json_2)
	AG.read(geo_json_1) do g1
		AG.getlayer(g1, 0) do l1
			AG.getfeature(l1, 0) do f1
				AG.getgeom(f1, 0) do g1
					AG.read(geo_json_2) do g2
						AG.getlayer(g2, 0) do l2
							AG.getfeature(l2, 0) do f2
								AG.getgeom(f2, 0) do g2
									return AG.toJSON(AG.intersection(g1, g2))
								end
							end
						end
					end
...
function union_geojson(geo_json_1, geo_json_2)
	AG.read(geo_json_1) do g1
		AG.getlayer(g1, 0) do l1
			AG.getfeature(l1, 0) do f1
				AG.getgeom(f1, 0) do g1
...
function intersect_geojson(geojson_object_vector::Vector{String})
	init_object = geojson_object_vector[1]
	for i in 2:length(geojson_object_vector)
		init_object = intersect_geojson(init_object, geojson_object_vector[i])
	end
	return init_object
end
...
function union_geojson(geojson_object_vector::Vector{String})
	init_object = geojson_object_vector[1]
	for i in 2:length(geojson_object_vector)
		init_object = union_geojson(init_object, geojson_object_vector[i])
	end
	return init_object
end
